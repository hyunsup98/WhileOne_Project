using UnityEngine;

public class EquipSlotPresenter
{
    private Player _model;          // 모델 (플레이어)
    private EquipSlotView _view;    // 뷰 (무기 장착 슬롯)

    public EquipSlotPresenter(Player model, EquipSlotView view)
    {
        _model = model;
        _view = view;

        // 모델(플레이어)을 담당한 담당자와 상의 후 Action 연결하기
        // 2. 무기 획득 시 서브 슬롯의 아이콘 변경

        // 선택중인 슬롯의 프레임 색 변경
        _model.Player_WeaponChange.onSwapWeapon += view.EquipWeapon;

        _model.Player_WeaponChange.onWeaponChanged += view.ChangeSubWeapon;

        // 내구도 UI 반영
        // _model.Player_WeaponChange._slotWeapon2.onDurabilityChanged += view.ChangeSubWeaponDurability;
    }
}