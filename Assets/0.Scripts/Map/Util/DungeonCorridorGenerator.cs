...
public static class DungeonCorridorGenerator
{
        ...
        GameObject corridorPrefabCross)
    {
        ...
        foreach (var position in dungeonGrid.GetAllPositions())
        {
            ...
            foreach (Vector2Int direction in directions)
            {
                ...
                // 복도 생성 (문 중심 연결)
                CreateCorridorBetweenRooms(room.roomObject, nextRoom.roomObject, direction, 
    ...
    /// <summary>
    /// 두 방 사이에 복도를 생성합니다 (문 중심 연결 방식).
    /// 방1의 DoorCenterMarker → 방2의 DoorCenterMarker를 직선으로 연결합니다.
    /// </summary>
    private static void CreateCorridorBetweenRooms(
        GameObject room1, 
        GameObject room2, 
        ...
        Vector2Int room2Pos)
    {
        ...
        Debug.Log($"[DungeonCorridorGenerator] 복도 생성 완료 - 방1: {room1.name}, 방2: {room2.name}, 복도 개수: {corridorCount}\n" +
            $"방1 DoorCenterMarker: {room1DoorPos}, 방2 DoorCenterMarker: {room2DoorPos}, 거리: {targetDistance}, 실제 프리팹 길이: {actualCorridorLength}");
    }
