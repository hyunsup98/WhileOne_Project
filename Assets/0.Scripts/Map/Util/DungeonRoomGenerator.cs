...
public static class DungeonRoomGenerator
{
    ...
    /// </summary>
    public static void ProcessPostGeneration(DungeonGrid dungeonGrid, FloorInfo floorInfo, out Vector2Int startRoomPosition, out Vector2Int exitRoomPosition)
    {
        startRoomPosition = Vector2Int.zero;
        exitRoomPosition = Vector2Int.zero;
        
        if (floorInfo == null)
        {
            Debug.LogWarning("[DungeonRoomGenerator] floorInfo가 null이어서 방 타입을 지정할 수 없습니다.");
            return;
        }
        ...
        // 방 거리 계산
        Dictionary<Vector2Int, int> distances = CalculateDistancesFrom(dungeonGrid, startRoomPosition);
        ...
        // out 매개변수를 람다에서 사용할 수 없으므로 지역 변수에 복사
